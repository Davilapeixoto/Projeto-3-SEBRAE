...
A criação de usuário continua com a regra original: a primeira conta é administradora e as demais são alunas.

Caso o banco já possua outras contas e seja necessário promover uma conta existente, use uma destas opções:

1. Defina a variável de ambiente `ADMIN_EMAIL` com o e-mail da conta e reinicie a aplicação.
2. Preencha `aplicacao.admin.email` em `application.properties`.
3. Execute o arquivo `banco/promover_usuario_admin.sql` após trocar o e-mail de exemplo.

Depois da promoção, encerre a sessão e entre novamente.
